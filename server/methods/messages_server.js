  ...
  'systemMsg': function(action, chatID){
    ...
    switch (action) {
      case 'Buy':
        ...
        var sysmsg = thisChat.buyerName.bold() + " wants to buy your " + thisChat.prodName.bold() + " for N" + thisPrice + ".";
        var msgFor = thisChat.seller;
        break;

      case 'Accept':
        var thisPrice = thisChat.agreedPrice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        var sysmsg = thisChat.sellerName.bold() + " accepts to sell this " + thisChat.prodName.bold() + " for N" + thisPrice + ".";
